...
class CassandraTest < Test::Unit::TestCase
  ...
  def setup
    @twitter = CassandraLegacy.new('Twitter', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @twitter.clear_keyspace!

    @blogs = CassandraLegacy.new('Multiblog', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @blogs.clear_keyspace!

    @blogs_long = CassandraLegacy.new('MultiblogLong', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @blogs_long.clear_keyspace!

    @type_conversions = CassandraLegacy.new('TypeConversions', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @type_conversions.clear_keyspace!

    CassandraLegacy::WRITE_DEFAULTS[:consistency] = CassandraLegacy::Consistency::ONE
    CassandraLegacy::READ_DEFAULTS[:consistency]  = CassandraLegacy::Consistency::ONE
  ...
  def test_setting_default_consistency
    assert_nothing_raised do
      @twitter.default_read_consistency = CassandraLegacy::Consistency::ALL
    end
    assert_equal(CassandraLegacy::READ_DEFAULTS[:consistency], CassandraLegacy::Consistency::ALL)

    assert_nothing_raised do
      @twitter.default_write_consistency = CassandraLegacy::Consistency::ALL
    end
    assert_equal(CassandraLegacy::WRITE_DEFAULTS[:consistency], CassandraLegacy::Consistency::ALL)
  end
...
#  end
  ...
  def test_disconnect_when_not_connected!
    assert_nothing_raised do
      @twitter = CassandraLegacy.new('Twitter', "127.0.0.1:9160", :retries => 2, :exception_classes => [])
      @twitter.disconnect!
    end
  end
  ...
  if CASSANDRA_VERSION.to_f >= 0.7
    ...
    def test_get_indexed_slices
      ...
      # Currently (as of CassandraLegacy 1.1) you can't use anything but == as the
    ...
    def test_column_family_mutation
      ...
      @twitter.add_column_family(
        CassandraLegacy::ColumnFamily.new(
          :keyspace => 'Twitter',
          :name     => k
        )
      )
  ...
  if CASSANDRA_VERSION.to_f >= 0.8
    ...
    def test_composite_column_type_conversion
      columns = {}
      @composites.push(
        CassandraLegacy::Composite.new_from_parts([[20].pack('N'), "meerkat"])
      )
      ...
      columns_in_order = [
        ...
        CassandraLegacy::Composite.new([5].pack('N'), "zebra"),
        CassandraLegacy::Composite.new([10].pack('N'), "kangaroo"),
        CassandraLegacy::Composite.new([20].pack('N'), "meerkat"),
      ]
      ...
      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :start => CassandraLegacy::Composite.new([1].pack('N')),
        :finish => CassandraLegacy::Composite.new([20].pack('N'))
      ).keys
      assert_equal(columns_in_order[0..-2], column_slice)

      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :start => CassandraLegacy::Composite.new([5].pack('N')),
        :finish => CassandraLegacy::Composite.new([5].pack('N'), :slice => :after)
      ).keys
      assert_equal(columns_in_order[1..2], column_slice)

      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :start => CassandraLegacy::Composite.new([5].pack('N'), :slice => :after).to_s
      ).keys
      assert_equal(columns_in_order[-2..-1], column_slice)

      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :finish => CassandraLegacy::Composite.new([10].pack('N'), :slice => :before).to_s
      ).keys
    ...
    def test_dynamic_composite_column_type_conversion
      ...
      column_slice = @type_conversions.get(:DynamicComposite, key,
        :start => CassandraLegacy::DynamicComposite.new(['i', [1].pack('N')]),
        :finish => CassandraLegacy::DynamicComposite.new(['i', [10].pack('N')])
      ).keys
      assert_equal(columns_in_order[0..-2], column_slice)

      column_slice = @type_conversions.get(:DynamicComposite, key,
        :start => CassandraLegacy::DynamicComposite.new(['IntegerType', [5].pack('N')]),
        :finish => CassandraLegacy::DynamicComposite.new(['IntegerType', [5].pack('N')], :slice => :after)
      ).keys
      assert_equal(columns_in_order[1..2], column_slice)

      column_slice = @type_conversions.get(:DynamicComposite, key,
        :start => CassandraLegacy::DynamicComposite.new(['i', [5].pack('N')], :slice => :after).to_s
      ).keys
      assert_equal([columns_in_order[-1]], column_slice)

      column_slice = @type_conversions.get(:DynamicComposite, key,
        :finish => CassandraLegacy::DynamicComposite.new(['i', [10].pack('N')], :slice => :before).to_s
      ).keys
      assert_equal(columns_in_order[0..-2], column_slice)
  ...
  def test_keyspace_operations
    system = CassandraLegacy.new 'system'
    keyspace_name = 'robots'
    keyspace_definition = CassandraLegacy::Keyspace.new :name => keyspace_name,
    ...
    system.add_keyspace keyspace_definition
    CassandraLegacy.new(keyspace_name).drop_keyspace
    assert system.keyspaces.none? {|it| it == keyspace_name }
  end
